...
import { Users, BookOpen, TrendingUp, UserCog, Download, ArrowRight, Flame } from 'lucide-react';
...
const kpis = [
  { label: 'Estudiantes',       value: kpiData.totalStudents,          sub: `+${kpiData.enrollmentGrowth}% este mes`, icon: Users,      color: C.primary },
  { label: 'Programas Activos', value: kpiData.activePrograms,         sub: 'Activos',                                icon: BookOpen,   color: C.gold    },
  { label: 'Educadores',        value: kpiData.totalEducators,         sub: 'Personal activo',                        icon: UserCog,    color: C.active  },
  { label: 'Asistencia',        value: `${kpiData.attendanceRate}%`,   sub: 'Promedio general',                       icon: TrendingUp, color: '#6b4040' },
  { label: 'Finalización',      value: `${kpiData.completionRate}%`,   sub: 'Programas completados',                  icon: TrendingUp, color: C.primary },
  { label: 'Crecimiento',       value: `+${kpiData.enrollmentGrowth}%`,sub: 'vs mes anterior',                        icon: Flame,      color: C.active  },
];
        ...
        <div style={{ flex: 1 }}>
          <SectionBlock title="INSCRIPCIONES POR CATEGORÍA" delay={0.25}>
            <ResponsiveContainer width="100%" height={240}>
              <PieChart>
                <Pie data={chartData.enrollmentByCategory} cx="50%" cy="50%" outerRadius={90} dataKey="value"
                  label={({ name, value }) => `${name}: ${value}`} labelLine={false}>
                  {chartData.enrollmentByCategory.map((entry: any, idx: number) => (
